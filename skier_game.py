...
class SkierClass(pygame.sprite.Sprite):  # SkierClass inherits from pygame.sprite.Sprite;
    ...
    def turn(self, num):
        ...
        self.direction = max(-2,
                             self.direction)  # here -2 means turning left two times, since we can turn left max 2 time
        # it is written in this way

        self.direction = min(2,
                             self.direction)  # same scenario with turning right, but it will be in min because of right
    ...
    for j in range(num):
        ...
        if location not in locations:
            ...
            attribute = random.choice(
                list(cfg.OBSTACLE_PATHS.keys()))  # returns a randomly selected element from the list
